...
function inputQty({product,productCountQty,setProductCountQty}) {
    ...
    return (
            <div key='inputQty' style={{ display: 'flex', alignItems: 'center' }}>
				<button className="decrement-btn text-24px" onClick={( event ) => handleQtyChange( event, 'decrement' )} >-</button>
				<input
					type="number"
					min="1"
					...
					readonly='readonly'
				/>
				<button className={` increment-btn text-20px `} disabled={maxQty} onClick={( event ) => handleQtyChange( event, 'increment' )}>+</button>
			</div>
    )
}
